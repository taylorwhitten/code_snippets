pair <- function (obj, data, method = NULL, mpower = 2, verbose = 0) 
{
  ...
  for (i in strataID) {
    ...
    if (length(idx) > 0) {
      if (is.null(method)) {
        ...
        mat <- as.matrix(dist(data[idx, obj$vars], method = method, 
                              p = mpower))
      }
  ...
  cat("\n")
  return(invisible(list(reservoir = reservoir, reservoir2 = reservoir2, 
                        paired = paired, full.paired = full.paired)))
}
